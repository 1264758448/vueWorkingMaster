    ...
    methods: {
        ...
        // 新增表单的勾选事件
            allDataDian(val){
                    ...
                    console.log(this.sizeForm.basicTruckDrivers,'xinzengbiaodan')
            },
            // 最终保存按钮
             add(){
                 this.sizeForm.addDate=new Date()
                this.$http.post("v1/driver/insert/",this.sizeForm).then(res=>{
                    console.log(res,'add')
                    if(res.status==200){
